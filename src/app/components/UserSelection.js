...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('algorithm_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('algorithm_quizUserId', newUserId);
    ...
    localStorage.setItem('algorithm_quizResults', JSON.stringify(updatedUsers));
