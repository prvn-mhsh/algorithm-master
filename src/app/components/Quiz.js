...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('algorithm_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('algorithm_quizResults') || '[]');
      ...
      localStorage.setItem('algorithm_quizResults', JSON.stringify(updatedResults));
    }
  };
